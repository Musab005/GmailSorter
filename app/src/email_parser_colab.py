...
#     creds = None
#     if os.path.exists(userdata.get('TOKEN_PATH')):
#         creds = Credentials.from_authorized_user_file(userdata.get('TOKEN_PATH'), SCOPES)
#     if not creds or not creds.valid:
#         if creds and creds.expired and creds.refresh_token:
#             creds.refresh(Request())
#         else:
#             flow = InstalledAppFlow.from_client_secrets_file(
#                 userdata.get('CREDENTIALS_PATH'), SCOPES)
#             creds = flow.run_local_server(port=0)
#         # save the new token
#         with open(userdata.get('TOKEN_PATH'), 'w') as token:
